...
#         for col in ['PLNT_CD','MTRL_CD','MTRL_PROD_HIERCY_4']:
#             df_old[col] = df_old[col].astype(int)
...
def get_smallest_distance(lst):
    ...
    if len(df_result)==0:
        ...
        df_result = df_result.append(df_aux, ignore_index=True)
    return df_result

def scale_data(df_new, df_old, col_nm):
    """
    ...
    # fit the scaler
    scaler.fit(df.drop(col_nm,axis=1))
    # apply scaling to the data frame
    scaled_features = scaler.transform(df.drop(col_nm,axis=1))
    # create a new data frame with the scaled data and get columns data from the old data frame
    df_feat = pd.DataFrame(scaled_features, columns=df.drop(col_nm,axis=1).columns)
    df = df[df.columns[:1]]
    # how we need the material code copy the columns from feature data frame to the original one
    ...
    df_new_scaled = df[0:old_len_new]
    df_old_scaled = df[old_len_new::]
...
def reverse_one_hot_encoder(df, col_nm):
    ...
    try:
        df_result = pd.concat(
            ...
            df_aux[df_aux==1].stack().reset_index().drop(0,1)['level_1']\
                .apply(lambda x: int(x.replace(col_nm+'_','')))],
            axis=1)
    except:
        df_result = pd.concat(
            [df.drop('level_0', axis=1).reset_index().drop('level_0', axis=1),
            df_aux[df_aux==1].stack().reset_index().drop(0,1)['level_1']\
                .apply(lambda x: int(x.replace(col_nm+'_','')))],
            axis=1)
    ...
    df_result = df_result.rename(columns={'level_1':col_nm})
    return df_result

def calculate_scaling_factor(df_new, df_result):
    """
    ...
    for i in range(len(df_new)):
        ...
        diff_arr = df_new.values[i] - df_result[df_new.columns].values[i]
    ...
    for euclidean_distance in range(len(lst_dist)):
        if euclidean_distance == 0:
            ...
            scaling_factor = round(np.exp(-(euclidean_distance**2/statistics.mean(lst_dist)**2)), 2)
...
def reverse_scaler(df_result, df_old, lst_cols_created_ohe):
    ...
    df_old_orig_ohe = df_old.iloc[df_result['index']][lst_cols_created_ohe].reset_index().drop('index',axis=1)
    # Homogenize the index values
    df_result_wthout_ohe.index = df_old_orig_ohe.index
    # Assign the columns.
    df_result_wthout_ohe[df_old_orig_ohe.columns] = df_old_orig_ohe[df_old_orig_ohe.columns]
...
def get_similar_rows(df_old, df_new, lst_ohe):
    """
    ...
    df_new_scaled, df_old_scaled = scale_data(df_new, df_old, 'MTRL_CD')
    ...
    df_result = calculate_scaling_factor(df_new=df_new_scaled, df_result=df_result)
    # reverse the Standar Scaler to be possible to reverse the one hot encoding process
    lst_cols_created_ohe = get_columns_createded_by_ohe(df=df_result, lst_ohe=lst_ohe)
    df_result = reverse_scaler(df_result, df_old, lst_cols_created_ohe)
    ...
    lst_orig_col = lst_orig_col.tolist()
    lst_orig_col.insert(0, "index")
    lst_orig_col.append("scaling_factor")
    df_result = df_result[lst_orig_col]
    return df_result
...
def execute_grouped(input_file_name, output_file_name, plnt_col_nm, grp_col_nm, update_base_ind = True):
    ...
    for plnt in df_new[plnt_col_nm].unique():
        ...
        for grp in df_new[df_new[plnt_col_nm]==plnt][grp_col_nm].unique():
            ...
            if len(df_old_aux):
                # define similarity
                df_result = get_similar_rows(df_old_aux, df_new_aux, lst_ohe=[plnt_col_nm,grp_col_nm])
                # export file
